...
{
	uint32_t pte1 = l1[Z_XTENSA_L1_POS(addr)];
	uint32_t *l2 = (void *) (pte1 & Z_XTENSA_PTE_VPN_MASK);

	return pte_valid(pte1) ? l2[(addr >> Z_XTENSA_PPN_SHIFT) & 0x3ff] : 0xf;
}
...
void xtensa_page_table_validate(uint32_t *l1)
{
	for (int i = 0; i < 1024; i++) {
		...
		__ASSERT_NO_MSG(Z_XTENSA_PTE_RING_GET(pte) == 0);

		uint32_t phys = lookup_pte(l1, pte & Z_XTENSA_PTE_VPN_MASK);
		ARG_UNUSED(phys);

		__ASSERT_NO_MSG(pte_valid(phys));
		__ASSERT_NO_MSG((phys >> 12) == (pte >> 12)); /* map to same page */
		__ASSERT_NO_MSG(Z_XTENSA_PTE_RING_GET(phys) == 0);
...
static void compute_regs(uint32_t user_asid, uint32_t *l1_page, struct tlb_regs *regs)
{
	...
	l1_page[Z_XTENSA_L1_POS(regs->ptevaddr)] = (uint32_t) l1_page | Z_XTENSA_MMU_CACHED_WB;
