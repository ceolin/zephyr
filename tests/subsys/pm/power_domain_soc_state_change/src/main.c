...
ZTEST(power_domain_soc_state_change_1cpu, test_power_domain_soc_state_change)
{
	...
	(void)pm_state_cpu_get_all(0, &cpu_states);

	/* Sleep to transition to first STATE: STANDBY */
	k_usleep(cpu_states[2].min_residency_us + cpu_states[2].exit_latency_us);
	...
	k_usleep(cpu_states[1].min_residency_us + cpu_states[1].exit_latency_us);
	...
	k_usleep(cpu_states[0].min_residency_us + cpu_states[0].exit_latency_us);
